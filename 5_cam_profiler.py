...
def parse_arguments():
    ...
    parser.add_argument('--cam-method', type=str, default='GradCAM',
                       choices=['GradCAM', 'HiResCAM', 'ScoreCAM', 'GradCAMPlusPlus', 'AblationCAM', 'XGradCAM', 'EigenCAM', 'FullGrad'],
                       help='CAM method to use (default: GradCAM)')
    
    parser.add_argument('--target-class', type=int, default=246,
                       help='Target class for CAM computation (default: 246 - boxer)')
...
def get_target_layers(model, model_name):
    """Get the target layers for CAM computation based on model architecture."""
    if model_name == "alexnet":
        # For AlexNet, use the last convolutional layer
        return [model.features[-1]]
    elif model_name in ["resnet18", "resnet101"]:
        # For ResNet, use the last convolutional layer before the final pooling
        return [model.layer4[-1]]
    else:
        raise ValueError(f"Target layers not defined for model: {model_name}")


def create_cam_object(model, target_layers, cam_method):
    """Create CAM object based on the specified method."""
    cam_classes = {
        'GradCAM': GradCAM,
        'HiResCAM': HiResCAM,
        'ScoreCAM': ScoreCAM,
    ...
    cam_class = cam_classes[cam_method]
...
def run_profiling(model, input_tensor, model_name, device, cam_method, target_class):
    """Run the CAM profiling session."""
    ...
    cam = create_cam_object(model, target_layers, cam_method)
    targets = [ClassifierOutputTarget(target_class)]
    
    print(f"CAM method: {cam_method}")
    print(f"Target layers: {[str(layer) for layer in target_layers]}")
                ...
                with mps_profiler.metal_capture(capture_file):
                    with profile(
                        ...
                        for step in range(12):
                            with record_function("cam_computation"):
                                _ = cam(input_tensor=input_tensor, targets=targets)
                            prof.step()
...
def main():
    ...
    print(f"  CAM Method: {args.cam_method}")
    ...
    # Run profiling
    run_profiling(model, input_tensor, args.model, device, args.cam_method, args.target_class)
